...
```mermaid
graph LR
    C[🔮 CONNECT<br/>creator.yaml → WHO] --> O[🧭 ORIENT<br/>STATE.yaml + .meta.yaml → WHERE]
    O --> R[⚡ REASON<br/>step-by-step, challenge obvious]
    R --> D[🎯 DELIVER<br/>tables, one action, mirror language]
    D -.->|self-check| C
```
